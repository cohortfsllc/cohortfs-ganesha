			 ...
			 struct nfs_resop4 *resp)
{
	...
	case LAYOUTRETURN4_FSID:
		...
		fsid = data->current_obj->attributes.fsid;
		return_fsid = true;
 ...
 again:
		...
		glist_for_each_safe(glist, glistn,
			...
			if (return_fsid) {
				fsal_fsid_t this_fsid = obj->attributes.fsid;
			       ...
			       bool *deleted)
{
	...
	recalls = glist_length(&fstate->layoutrecall_list);
	...
	if (circumstance != circumstance_reclaim) {
		...
		glist_for_each_safe(seg_iter, seg_next,
			...
			handle_recalls(arg, fstate, layout_state,
				       &g->sls_segment);
