		   ...
		   struct nfs_resop4 *resp)
{
	...
	if (FSAL_IS_ERROR(status)) {
		res_LOOKUP4->status = fsal_error_convert(status);
		goto out;
	}

	if (file_obj->type == DIRECTORY && file_obj->junction_export != NULL) {
		/* Handle junction */
		 ...
		 * junction.
		 */
		if (!export_ready(file_obj->junction_export)) {
		...
		get_gsh_export_ref(file_obj->junction_export);
		...
		op_ctx->export = file_obj->junction_export;
	...
	if (!nfs4_FSALToFhandle(&data->currentFH, file_obj, op_ctx->export)) {
