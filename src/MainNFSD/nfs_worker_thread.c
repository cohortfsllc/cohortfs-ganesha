 ...
 *
 * @param preq [INOUT] pointer to request_data_t (of type nfs)
 *
...
static void nfs_rpc_execute(request_data_t *preq, nfs_worker_data_t *pworker_data)
{
  ...
  dpq_status = nfs_dupreq_add_not_finished(req, arg_nfs, &res_nfs);
  switch(dpq_status)
    {
  ...
  if (export_check_result == EXPORT_PERMISSION_DENIED)
      ...
      if(req->rq_prog == nfs_param.core_param.program[P_NFS])
        {
          if(req->rq_vers == NFS_V2)
            {
              /* All the nfs_res structure in V2 have the status at the same place
  ...
  else  /* export_check_result == EXPORT_PERMISSION_GRANTED is TRUE */
    {
      ...
      rc = pworker_data->pfuncdesc->service_function(arg_nfs, 
						     pexport, 
						     pfsal_op_ctx,
                                                     pworker_data,
                                                     req, 
