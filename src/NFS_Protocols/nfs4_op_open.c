...
int nfs4_op_open(struct nfs_argop4 *op, compound_data_t * data, struct nfs_resop4 *resp)
{
  ...
  if ((rc = state_open_owner_begin41(clientid,
				     arg_OPEN4.owner,
				     &transaction)) != 0)
    {
      res_OPEN4.status = staterr2nfs4err(rc);
      return res_OPEN4.status;
    }
  ...
  if (data->pexport->fs_static_info->lock_support | FSAL_LOCKS_POSIX)
    {
