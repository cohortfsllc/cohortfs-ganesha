			    ...
			    const char *path, struct fsal_obj_handle **obj_pub)
{
	...
	struct cohort_export *export =
	    container_of(op_ctx->fsal_export, struct cohort_export, export);
 ...
 * .., which is ironic since the Cohort readdir call synthesizes them
				  ...
				  fsal_readdir_cb cb, bool *eof)
{
	...
	while (!(*eof)) {
		...
		rc = ceph_readdirplus_r(export->cmount, dir_desc, &de, &st,
					&stmask);
		if (rc < 0) {
			fsal_status = cohort2fsal_error(rc);
			goto closedir;
		} else if (rc == 1) {
			/* skip . and .. */
				   ...
				   bool refresh)
{
	...
	struct cohort_export *export =
	    container_of(op_ctx->fsal_export, struct cohort_export, export);
	/* The private 'full' directory handle */
	struct cohort_handle *link = container_of(link_pub, struct cohort_handle, handle);
	/* Pointer to the Cohort link content */
...
static fsal_status_t getattrs(struct fsal_obj_handle *handle_pub)
{
	...
	cohort2fsal_attributes(&st, &handle->handle.attributes);
				 ...
				 const char *new_name)
{
	...
	struct cohort_export *export =
	    container_of(op_ctx->fsal_export, struct cohort_export, export);
	/* The private 'full' object handle */
	struct cohort_handle *olddir =
	    container_of(olddir_pub, struct cohort_handle, handle);
	/* The private 'full' destination directory handle */
				...
				bool *fsal_stable)
{
	...
	if (nb_written < 0)
		return cohort2fsal_error(nb_written);
