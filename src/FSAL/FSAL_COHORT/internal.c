...
/**
 * @brief Convert a struct stat from Cohort to a struct attrlist
 *
 ...
 *
 * This function constructs a new Cohort FSAL object handle and attaches
		     ...
		     struct cohort_export *export, struct cohort_handle **obj)
{
	...
	struct cohort_handle *constructing = NULL;

	assert(i);
	*obj = NULL;

	constructing = gsh_calloc(1, sizeof(struct cohort_handle));
...
// XXX	constructing->vi.snapid.val = st->st_dev;
	...
	cohort2fsal_attributes(st, &constructing->handle.attributes);
