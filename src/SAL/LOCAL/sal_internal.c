...
prealloc_pool entryheaderpool;
#ifdef _USE_FSALMDS
prealloc_pool layoutentrypool;
#endif
prealloc_pool statepool;
prealloc_pool ownerpool;
...
state_t* newstate(clientid4 clientid, entryheader_t* header)
{
    ...
    GetFromPool(state, &statepool, state_t);
...
/*		    rc = pthread_mutex_lock(&(owner->mutex)); */
    ...
    else if (rc == HASHTABLE_ERROR_NO_SUCH_KEY)
	{
	    ...
	    GetFromPool(owner, &ownerpool, state_owner_t);
